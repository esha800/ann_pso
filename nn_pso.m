...
%initialize the domain for input variable
for j=1:kk
    LB(1,j)=-1.5;
    UB(1,j)=1.5;
end
%creating particle random population
pop=10;
for i=1:pop
    for j=1:kk
...
disp('RMSE:');hold on
err2=sqrt(sum((net_f(inputs)-targets).^2)/length(net_f(inputs)))
disp('MBE:');hold on
err3=(sum(net_f(inputs)-targets))/length(net_f(inputs))

disp('MAPE:');
err4=100*(sum((abs(net_f(inputs)-targets))/targets))/length(net_f(inputs))
%Regression plot
